...
loc = fiblat(npoints)
maps = ducc0.sht.synthesis_general(alm=alm, loc=loc, lmax=lmax, mmax=mmax, spin=spin, epsilon=1e-12, nthreads=nthreads)
#maps =np.random.uniform(0.,1.,maps.shape)
t0=time()
res=ducc0.sht.pseudo_analysis_general(map=maps, loc=loc, lmax=lmax, mmax=mmax, spin=spin, epsilon=epsilon, nthreads=nthreads, maxiter=maxiter)
dt = time()-t0
...
if res[1] == 7:
    ...
    print("unclear error")
maps2 = ducc0.sht.synthesis_general(alm=res[0], loc=loc, lmax=lmax, mmax=mmax, spin=spin, epsilon=1e-12, nthreads=nthreads)
