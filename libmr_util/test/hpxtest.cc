...
/*
 *  Copyright (C) 2004-2020 Max-Planck-Society
...
#include "ducc0/constants.h"
//#include "alm.h"
//#include "alm_healpix_tools.h"
//#include "alm_powspec_tools.h"
#include "ducc0/geom_utils.h"
#include "ducc0/timers.h"
//#include "announce.h"
#include "ducc0/compress_utils.h"
#include "Healpix_cxx/moc.h"
//#include "moc_fitsio.h"
#include "ducc0/crangeset.h"
...
#ifdef UNITTESTS
int errcount=0;
#define FAIL(a) {a; if (++errcount>100) MR_fail("unit test errors");}
#else
#define FAIL(a) a;
#endif
...
static mt19937 engine{randseed};
uniform_real_distribution<double> rrand(0.,1.);
double frand() { return rrand(engine); }
...
template<typename I> void check_rangeset()
  ...
  rangeset<I> b;
  b.append(1,11);
  MR_assert(b==makeRS<I>("1 11"),"mismatch");
  b.append(10,15);
  MR_assert(b==makeRS<I>("1 15"),"mismatch");
  b.append(1,15);
  MR_assert(b==makeRS<I>("1 15"),"mismatch");
  b.append(7,15);
  MR_assert(b==makeRS<I>("1 15"),"mismatch");
  ...
  rangeset<I> b;
  b.add(5, 11);
  MR_assert(b==makeRS<I>("5 11"),"error");
  b.add(1, 7);
  MR_assert(b==makeRS<I>("1 11"),"error");
  b.add(1, 11);
  MR_assert(b==makeRS<I>("1 11"),"error");
  b.add(30, 41);
  MR_assert(b==makeRS<I>("1 11 30 41"),"error");
  b.add(1, 11);
  MR_assert(b==makeRS<I>("1 11 30 41"),"error");
  b.add(-1,0);
  MR_assert(b==makeRS<I>("-1 0 1 11 30 41"),"error");
  b.add(-2,-1);
  MR_assert(b==makeRS<I>("-2 0 1 11 30 41"),"error");
  b.add(-2,-1);
  MR_assert(b==makeRS<I>("-2 0 1 11 30 41"),"error");
  b.add(2, 11);
  MR_assert(b==makeRS<I>("-2 0 1 11 30 41"),"error");
  ...
  MR_assert(b==makeRS<I>("-2 0 1 11 30 41"),"error");
  b.add(15, 21);
  MR_assert(b==makeRS<I>("-2 0 1 11 15 21 30 41"),"error");
  }
  {
  rangeset<I> b = makeRS<I>("0 11 20 31");
  b.remove(5,25);
  MR_assert(b==makeRS<I>("0 5 25 31"),"error");
  b.remove(31,32);
  MR_assert(b==makeRS<I>("0 5 25 31"),"error");
  b.remove(35,38);
  MR_assert(b==makeRS<I>("0 5 25 31"),"error");
  ...
  MR_assert(b==makeRS<I>("0 5 25 31"),"error");
  b.remove(27,29);
  MR_assert(b==makeRS<I>("0 5 25 27 29 31"),"error");
  b.remove(25,26);
  MR_assert(b==makeRS<I>("0 5 26 27 29 31"),"error");
  b.remove(4,6);
  MR_assert(b==makeRS<I>("0 4 26 27 29 31"),"error");
  b.remove(-20,40);
  MR_assert(b==makeRS<I>(""),"error");
  b.remove(-20,40);
  MR_assert(b==makeRS<I>(""),"error");
  }
  {
  rangeset<I> b = makeRS<I>("0 11 20 31");
  b.intersect(2,29);
  MR_assert(b==makeRS<I>("2 11 20 29"),"error");
  b.intersect(-8,50);
  MR_assert(b==makeRS<I>("2 11 20 29"),"error");
  b.intersect(2,50);
  MR_assert(b==makeRS<I>("2 11 20 29"),"error");
  ...
  MR_assert(b==makeRS<I>("2 11 20 29"),"error");
  b.intersect(3,11);
  MR_assert(b==makeRS<I>("3 11"),"error");
  b = makeRS<I>("0 11 20 31");
  b.intersect(3,15);
  MR_assert(b==makeRS<I>("3 11"),"error");
  b = makeRS<I>("0 11 20 31");
  b.intersect(17,30);
  MR_assert(b==makeRS<I>("20 30"),"error");
  b = makeRS<I>("0 11 20 31");
  b.intersect(11,20);
  MR_assert(b==makeRS<I>(""),"error");
  b = makeRS<I>("0 11 20 31");
  ...
  {
  MR_assert(makeRS<I>("1 11 20 31 40 56")==
                makeRS<I>("20 31 40 51").op_or
                (makeRS<I>("1 11 45 56")),"error");
  MR_assert(makeRS<I>("1 11 45 56")==
                makeRS<I>("").op_or
                (makeRS<I>("1 11 45 56")),"error");
  MR_assert(makeRS<I>("1 11 45 56")==
  ...
  {
  MR_assert(makeRS<I>("22 24 45 51")==
                makeRS<I>("20 31 40 51").op_and
                (makeRS<I>("1 11 22 24 45 56")),"error");
  MR_assert(makeRS<I>("20 31 40 51 90 101 110 121 200 201")==
  ...
  {
  MR_assert(makeRS<I>("20 31 40 45")==
  ...
  MR_assert(b.contains(makeRS<I>("20 31 40 51")),"error");
  MR_assert(b.contains(makeRS<I>("20 21")),"error");
  MR_assert(b.contains(makeRS<I>("50 51")),"error");
  MR_assert(!b.contains(makeRS<I>("19 31 40 51")),"error");
  MR_assert(!b.contains(makeRS<I>("20 31 40 52")),"error");
  MR_assert(!b.contains(makeRS<I>("20 51")),"error");
  MR_assert(!b.contains(makeRS<I>("0 1")),"error");
  MR_assert(!b.contains(makeRS<I>("0 20 31 40 51 100")),"error");
  }
  {
  rangeset<I> b = makeRS<I>("20 31 40 51");
  ...
  MR_assert(b.overlaps(makeRS<I>("20 31 40 51")),"error");
  MR_assert(b.overlaps(makeRS<I>("20 21")),"error");
  MR_assert(b.overlaps(makeRS<I>("50 51")),"error");
  MR_assert(b.overlaps(makeRS<I>("19 31 40 51")),"error");
  MR_assert(b.overlaps(makeRS<I>("20 31 40 52")),"error");
  MR_assert(b.overlaps(makeRS<I>("20 51")),"error");
  MR_assert(!b.overlaps(makeRS<I>("0 1")),"error");
  MR_assert(!b.overlaps(makeRS<I>("0 20 31 40 51 100")),"error");
  }
  {
...
//   {
//   {
//   auto moc = read_Moc_from_fits<int>("sdssmoc.fits");
//   cout << moc.Rs().size() << endl;
//   auto r1 = moc.toCompressed();
//   cout << r1.size() << endl;
//   auto r2 = moc.toCompact();
//   auto x2=Moc<int>::fromCompact(r2);
//   cout << (x2==moc) << endl;
//   auto r3 = moc.toCompact2();
//   auto x3=Moc<int>::fromCompact2(r3);
//   cout << (x3==moc) << " " << moc.Rs().size() << " " << x3.Rs().size() << endl;
//   cout << r1.size() << " " << r2.size() << " " << r3.size() << endl;
//   }
//   {
//   auto moc = read_Moc_from_fits<int>("galex_moc.fits");
//   cout << moc.Rs().size() << endl;
//   auto r1 = moc.toCompressed();
//   cout << r1.size() << endl;
//   auto r2 = moc.toCompact();
...
template<typename I> void check_ringnestring()
  ...
  for (int order=0; order<=T_Healpix_Base<I>::order_max; ++order)
    ...
    uniform_int_distribution<I> irand(0,base.Npix()-1);
...
template<typename I> void check_pixzphipix()
  ...
  for (int order=0; order<=omax; ++order)
    ...
    uniform_int_distribution<I> irand(0,base1.Npix()-1);
    for (int m=0; m<nsamples; ++m)
      {
      double z,phi;
      I pix = irand(engine);
...
template<typename I> void check_neighbors()
  ...
  for (int order=0; order<=omax; ++order)
    ...
    for (int m=0; m<nsamples/10; ++m)
      ...
      MR_assert((check<=1)||((order==0)&&(check<=2)),"too few neighbors");
      }
    }
  for (I nside=3; nside<(I(1)<<omax); nside+=nside/2+1)
    {
...
void check_issue_229 (Ordering_Scheme scheme)
  ...
  int order=8;
  Healpix_Map<char> map (order,scheme);
  map.fill(false);
...
void check_query_disc_strict (Ordering_Scheme scheme)
  {
...
//   {
//   MR_assert(a.conformable(b), "maps are not conformable");
//   double res=0;
...
//   for (size_t i=0; i<rwgt.size(); ++i) rwgt[i]=rwgt0[i]+1.;
...
void check_isqrt()
  ...
  uint64_t val=uint64_t(0xF234)<<16, valsq=val*val;
...
template<typename I>void perf_neighbors(const string &name,
  ...
  cout << name << ": " << cnt/timer()*1e-6 << "MOps/s" << endl;
  }
template<typename I>void perf_pix2ang(const string &name,
  Ordering_Scheme scheme, double &dummy)
  {
  size_t cnt=0;
  SimpleTimer timer;
  int omax=T_Healpix_Base<I>::order_max;
...
template<typename I>void perf_query_disc(const string &name,
  ...
  cout << name << ": " << cnt/timer()*1e-3 << "kOps/s" << endl;
  }
...
//   check_fullweights();
#ifdef UNITTESTS
  if (errcount>0) MR_fail("unit test errors");
#endif
  }
