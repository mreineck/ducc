...
t0=time()
c = finufft.nufft2d2(coord[:,0], coord[:,1], f, nthreads=nthreads,eps=epsilon,debug=1)
print(time()-t0)

bla = ducc0.nufft.u2nu(grid=f.real, coord=coord, forward=False, epsilon=epsilon, nthreads=nthreads)
