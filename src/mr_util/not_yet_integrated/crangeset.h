...
template<typename T> class crangeset
    ...
    class RsIter
      ...
      RsIter(const crangeset &ref_) : idx(0), extra(false), ref(ref_), val(0)
        ...
        void advance_up_to(T goal)
          {
          using namespace std;
          ptrdiff_t idx2=ref.iiv(goal-1);
          if (idx2>ptrdiff_t(idx))
            {
            idx=idx2;
            extra=false;
            val=abs(ref.r[idx]);
  ...
  public:
    class IvIter
      ...
      public:
        ...
        ptrdiff_t ivlen() const { return e-b; }
        IvIter &operator++ ()
          {
          ++rsi;
    ...
    ptrdiff_t iiv (const T &val) const
      {
      return ptrdiff_t(std::upper_bound(r.begin(),r.end(),val,abscomp())
                   -r.begin())-1;
      }
  ...
  public:
    ...
    void checkConsistency() const
      ...
      using namespace std;
      if (r.size()==0) return;
      MR_assert(r[0]>=0,"incorrect first element in range set");
      for (size_t i=1; i<r.size(); ++i)
        MR_assert(abs(r[i])>abs(r[i-1]+1),"inconsistent entries");
      }
    ...
    void append(const T &v1, const T &v2)
      ...
      MR_assert(v1>=lastbegin,"bad append operation");
    ...
    bool contains (T a,T b) const
      ...
      if (r[res]<0)
        {
        if (res==ptrdiff_t(r.size())-1) return false; // beyond end
        if (r[res+1]>=0) return false; // long "off" range
        return ((a>-r[res])&&(b<=-r[res+1])); // mixed range
        }
      // r[res]>=0
      if ((res==ptrdiff_t(r.size())-1) || (r[res+1]>=0)) // short interval
    ...
    bool contains (T v) const
      ...
      if ((res<ptrdiff_t(r.size())-1) && (r[res+1]<0))
        return true;
      return (r[res]==v);
      }
