...
class Baselines_new
  ...
  public:
    Baselines_new() = default;
    template<typename T> Baselines_new(
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename Tms_in=cmav<complex<Tms>,1>> class Wgridder_new
  {
  private:
    constexpr static int log2tile=is_same<Tacc,float>::value ? 5 : 4;
    bool gridding;
...
timers.pop();
    ...
    template<size_t supp, bool wgrid> class HelperX2g2
      ...
      public:
        ...
        HelperX2g2(const Wgridder_new *parent_, const vmav<complex<Tcalc>,2> &grid_,
          vector<Mutex> &locks_, double w0_=-1, double dw_=-1)
          : parent(parent_), tkrn(*parent->krn), grid(grid_),
            iu0(-1000000), iv0(-1000000),
            bu0(-1000000), bv0(-1000000),
    ...
    template<size_t supp, bool wgrid> class HelperG2x2
      ...
      public:
        ...
        HelperG2x2(const Wgridder_new *parent_, const cmav<complex<Tcalc>,2> &grid_,
          double w0_=-1, double dw_=-1)
          : parent(parent_), tkrn(*parent->krn), grid(grid_),
            iu0(-1000000), iv0(-1000000),
            bu0(-1000000), bv0(-1000000),
...
//            if (mask(irow,ichan) && (wgt(irow, ichan)!=0) && (norm(ms_in(irow,ichan)!=0)))
  ...
  public:
    Wgridder_new(const cmav<double,2> &uvw,
      ...
      {
      timers.push("Baseline construction");
      bl = Baselines_new(uvw, freqlist_id, freqlist_nfreqs, freqlist_freqs, flip_u, flip_v, flip_w);
      MR_assert(bl.Nrows()<(uint64_t(1)<<32), "too many rows in the MS");
 //     MR_assert(bl.Nchannels()<(uint64_t(1)<<16), "too many channels in the MS");
      timers.pop();
      scanData();
...
template<typename Tcalc, typename Tacc, typename Tms, typename Tms_in=cmav<complex<Tms>,1>, typename Timg>
  ...
  Wgridder_new<Tcalc, Tacc, Tms, Timg, Tms_in> par(uvw, freqlist_id, freqlist_nfreqs, freqlist_freqs, ms, ms_out, dirty_in, dirty, wgt, mask, pixsize_x,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Tms_in=cmav<complex<Tms>,2>, typename Timg> void ms2dirty(const cmav<double,2> &uvw,
  ...
  ms2dirty_new<Tcalc, Tacc, Tms, cmav<complex<Tms>,1>, Timg>(uvw, freqlist_id, freqlist_nfreqs, freq, msnew, wgt, mask,  pixsize_x, pixsize_y, epsilon,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg> void dirty2ms_new(const cmav<double,2> &uvw,
  ...
  auto wgt(wgt_.size()!=0 ? wgt_ : wgt_.build_uniform(ms.shape(), 1.));
  auto mask(mask_.size()!=0 ? mask_ : mask_.build_uniform(ms.shape(), 1));
  Wgridder_new<Tcalc, Tacc, Tms, Timg> par(uvw, freqlist_id, freqlist_nfreqs, freqlist_freqs, ms_in, ms, dirty, dirty_out, wgt, mask, pixsize_x,
    pixsize_y, epsilon, do_wgridding, nthreads, verbosity, flip_u, flip_v, flip_w,
    divide_by_n, sigma_min, sigma_max, center_x, center_y, allow_nshift);
  }
template<typename Tcalc, typename Tacc, typename Tms, typename Timg> void dirty2ms(const cmav<double,2> &uvw,
  ...
  MR_assert(mask_.contiguous()||(mask_.size()==0), "oops3");
  auto mask(mask_.size()!=0 ? cmav<uint8_t,1>(&mask_(0,0), msnew.shape()) : cmav<uint8_t,1>::build_uniform(msnew.shape(), 1));
  dirty2ms_new<Tcalc, Tacc, Tms, Timg>(uvw, freqlist_id, freqlist_nfreqs, freq, dirty, wgt, mask,  pixsize_x, pixsize_y, epsilon,
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg> void dirty2ms_faceted(size_t nfx,size_t nfy, const cmav<double,2> &uvw,
  ...
  vmav<complex<Tms>,2> ms2(ms.shape(), PAGE_IN(nthreads));
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg, typename Tms_in=cmav<complex<Tms>,2>> void ms2dirty_tuning(const cmav<double,2> &uvw,
  ...
  if (xodd || yodd)  // odd image size
    {
    vmav<Timg,2> tdirty({npix_x+xodd, npix_y+yodd}, PAGE_IN(nthreads));
  ...
  if (bin.size()==0)
    ...
    {
    auto mask(mask_.size()!=0 ? mask_ : mask_.build_uniform(ms.shape(), 1));
    vmav<uint8_t,2> mask2({uvw.shape(0),freq.shape(0)}, PAGE_IN(nthreads));
    ...
    ms2dirty_faceted<Tcalc,Tacc>(nfx, nfy, uvw, freq, ms, wgt_, mask2, pixsize_x, pixsize_y, epsilon,
             do_wgridding, nthreads, dirty, verbosity, flip_u, flip_v, flip_w, divide_by_n,
             sigma_min, sigma_max, center_x, center_y);
    vmav<Timg,2> dirty2(dirty.shape(), PAGE_IN(nthreads));
    mav_apply([&](uint8_t i1, uint8_t i2, uint8_t &out) { out = (i1!=0) && (i2<icut_local); }, nthreads, mask, bin, mask2);
...
template<typename Tcalc, typename Tacc, typename Tms, typename Timg> void dirty2ms_tuning(const cmav<double,2> &uvw,
  ...
  if (bin.size()==0)
    ...
    mav_apply([&](uint8_t i1, uint8_t i2, uint8_t &out) { out = (i1!=0) && (i2<icut_local); }, nthreads, mask, bin, mask2);
    vmav<complex<Tms>,2> tms(ms.shape(), PAGE_IN(nthreads));
