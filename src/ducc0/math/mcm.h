...
template<typename Tout> void coupling_matrix_spin0_square(const cmav<double,2> &spec,
  ...
  auto lmax_spec = spec.shape(1)-1;
  MR_assert(mat.shape(0)==nspec, "number of spectra and matrices mismatch");
  MR_assert(mat.shape(1)==lmax+1, "bad number of matrix rows");
  MR_assert(mat.shape(2)==lmax+1, "bad number of matrix columns");
  using Tsimd = native_simd<double>;
  constexpr size_t vlen = Tsimd::size();
  auto lmax_spec_used = min(2*lmax, lmax_spec);
  auto spec2(vmav<double,2>::build_noncritical({nspec, lmax_spec_used+1+vlen-1}, PAGE_IN(nthreads)));
        ...
        if (el3min<=int(lmax_spec))
          ...
          if (nspec==1)
            ...
            for (size_t k=0; k<vlen; ++k)
              if (el2+k<=lmax)
                {
                mat(0, el1, el2+k) = Tout(val[k]);
                mat(0, el2+k, el1) = Tout(val[k]);
                }
            }
          else if (nspec==2)
            {
            ...
            for (size_t ispec=0; ispec<nspec; ++ispec)
              for (size_t k=0; k<vlen; ++k)
                if (el2+k<=lmax)
                  {
                  mat(ispec, el1, el2+k) = Tout(val[ispec][k]);
                  mat(ispec, el2+k, el1) = Tout(val[ispec][k]);
                  }
            }
          else if (nspec==3)
            {
          ...
          for (size_t ispec=0; ispec<nspec; ++ispec)
            for (size_t k=0; k<vlen; ++k)
              if (el2+k<=lmax)
                {
                mat(ispec, el1, el2+k) = Tout(0);
                mat(ispec, el2+k, el1) = Tout(0);
                }
        }
      }
    });
...
template<size_t opmask, typename Tout> void coupling_matrix_spin0and2_new(
  ...
  MR_assert(mat.shape(1)==lmax+1, "bad number of output matrix rows");
  MR_assert(mat.shape(2)==lmax+1, "bad number of output matrix columns");
  auto lmax_spec = spec.shape(1)-1;
  using Tsimd = native_simd<double>;
  constexpr size_t vlen = Tsimd::size();
  auto lmax_spec_used = min(2*lmax, lmax_spec);
...
//          FIXME: use generic lambdas in C++20
          if (nspec==1)
            ...
            for (int el3=el3min; el3<=maxidx; el3+=2)
              ...
              const Tsimd w00=w0*w0, w01=w0*w1, w11=w1*w1;
              const Tsimd w11p1=wp1[el3+1]*wp1[el3+1];
              Tsimd sp = Tsimd(&spec2(0,el3), element_aligned_tag());
            ...
            for (size_t k=0; k<vlen; ++k)
              if (el2+k<=lmax)
                ...
                if (op==4)
                  {
                  mat(0, el1, el2+k) = Tout(val[2][k]);
                  mat(0, el2+k, el1) = Tout(val[2][k]);
                  mat(1, el1, el2+k) = Tout(val[3][k]);
                  mat(1, el2+k, el1) = Tout(val[3][k]);
                  }
                else
                  {
                  mat(0, el1, el2+k) = Tout(val[op][k]);
