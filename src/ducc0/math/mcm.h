...
template<typename Tsimd, size_t nspec> inline array<Tsimd,nspec> sum_wigEB_new
  ...
  for (size_t i=0; i<vlen; ++i)
    el2v[i] = double(el2+i);
  for (int i=1, ofs=0; i<=max_i; ++ofs, i+=2)
    {
    int el3 = el3min+i;
    Tsimd j3val = w3j.get_EB(el1,el2,ofs);
    for (size_t ispec=0; ispec<nspec; ++ispec)
      val[ispec] += j3val*Tsimd(&spec2(ispec,el3), element_aligned_tag());
    }
  return val;
    ...
    while (auto rng=sched.getNext()) for(int el1=int(rng.lo); el1<int(rng.hi); ++el1)
      ...
      for (int el2=el1; el2<=((el1<=int(lsmall))?int(lmax):el1); el2+=vlen)
        ...
        if (el3min<=int(lmax_spec))
          {
          if (nspec==1)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wig00_new<Tsimd,1>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==2)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wig00_new<Tsimd,2>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==3)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wig00_new<Tsimd,3>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==4)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wig00_new<Tsimd,4>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec<=50)
            {
            array<Tsimd,50> val;
            sum_wig00_new<Tsimd>(el1, el2, lmax_spec, nspec, w3j, spec2, val); 
            store_mat<Tsimd>(el1, el2, s1, s2, nspec, mat, diag, val);
            }
          else
            {
...
else
  {
      for (int el2=el1; el2<=((el1<=int(lsmall))?int(lmax):el1); el2+=vlen)
        ...
        if (el3min<=int(lmax_spec))
          {
          if (nspec==1)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wigpp_new<Tsimd,1>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==2)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wigpp_new<Tsimd,2>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==3)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wigpp_new<Tsimd,3>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec==4)
            store_mat(el1, el2, s1, s2, mat, diag, sum_wigpp_new<Tsimd,4>(el1, el2, lmax_spec, w3j, spec2));
          else if (nspec<=50)
            {
            array<Tsimd,50> val;
            sum_wigpp_new<Tsimd>(el1, el2, lmax_spec, nspec, w3j, spec2, val); 
            store_mat<Tsimd>(el1, el2, s1, s2, nspec, mat, diag, val);
            }
          else
            {
