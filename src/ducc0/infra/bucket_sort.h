...
template<typename Tidx, typename Tkey> void subsort
  (vector<Tidx> &idx, Tkey *keys, size_t keybits, size_t lo,
  ...
  for (size_t i=0; i<nval; ++i)
    ...
    keybak[i] = keys[i];
    idxbak[i] = idx[i+lo];
    ++numbers[(keys[i]>>keyshift)&keymask];
    }
  Tidx ofs=0;
  for (auto &x: numbers)
    {
  ...
  for (size_t i=0; i<nkeys; ++i)
    {
    Tidx ilo=(i==0) ? 0 : numbers[i-1],
         ihi=numbers[i];
    subsort(idx, keys+ilo, keybits, lo + ilo,
      lo+ihi, newnumbers, idxbak, keybak);
    }
  }

//FIXME: change all externally visible vectors to mavs!
template<typename Tidx, typename Tkey> vector<Tidx> bucket_sort
  ...
  execDynamic(nkeys, nthreads, 1, [&](Scheduler &sched)
    {
    vector<Tidx> newnumbers;
    vector<Tkey> keys2, keybak;
    vector<Tidx> idxbak;
    while (auto rng=sched.getNext())
      for(auto i=rng.lo; i<rng.hi; ++i)
        {
        Tidx ilo=(i==0) ? 0 : numbers[nthreads-1].v[i-1],
             ihi=numbers[nthreads-1].v[i];
        if (keys2.size()<ihi-ilo) keys2.resize(ihi-ilo);
        for (size_t ii=ilo; ii<ihi; ++ii)
          keys2[ii-ilo] = keys[res[ii]];
        subsort(res, &keys2[0], keybits, ilo, ihi,
